...
void CollectExistingCode::LogBytecodeHandler(interpreter::Bytecode bytecode,
                                interpreter::OperandScale operand_scale,
                                Code* code) {
   ...
   if (shared->script()->IsScript()) {
    ...
    if (script->name()->IsString()) {
      ...
      if (line_num > 0) {
        ...
        if (listener_) {
        listener_->CodeCreateEvent(Logger::ToNativeByScript(
    ...
    if (listener_) {
      listener_->CodeCreateEvent(CodeEventListener::LAZY_COMPILE_TAG, *code, *shared,
                              isolate_->heap()->empty_string());
    } else {
      PROFILE(isolate_,
